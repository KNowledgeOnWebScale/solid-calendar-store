...
import {
  ...
  vacationStoreConfig,
  vacationStoreAlternateConfig, vacationStoreTurtleConfig
} from "./common";
...
describe("VacationStore", function () {
  ...
  describe("Vacation days present", () => {
  ...
  describe("No vacation days present", function () {
    const cssServer = new CssServer();
    const icalServer = new IcalServer({
      events: []
    });
    ...
    before(async () => {
      await cssServer.start(vacationStoreTurtleConfig);
      icalServer.start();
    });
    ...
    describe('No days are returned', () => {
      ...
      it("Turtle", async () => {
        const result = await getEndpoint("vacation", 'text/turtle');
        expect(result[0].replace(/\n/g, '')).to.deep.equal(`<https://pheyvaer.pod.knows.idlab.ugent.be/vacation-calendar#> a <https://data.knows.idlab.ugent.be/person/office/#VacationCalendar> .`);
      });
    });
  });
});
